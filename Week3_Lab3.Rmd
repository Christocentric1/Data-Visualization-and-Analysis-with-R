...
```{r}
consumptionNoOil <- read.csv("consumptionNoOil.csv")
...
p
```
Creating a histogram with the pressureTemp dataset
```{r}
pressureTemp <- read.csv("pressureTemp.csv", header=T)
p <- ggplot(pressureTemp, aes(Pressure))
p <- p + geom_histogram(colour="blue", fill="green", binwidth=5)
p
```
Loading the dataset ProvenOilReservesZerosT
```{r}
...
p
```
if we want the squares filled in dark blue
```{r}
p <- ggplot(ProvenOilReservesZerosT, aes(MTBarrels, Denmark))
...
```{r}
p <- ggplot(ProvenOilReservesZerosT, aes(MTBarrels, Norway, size=Norway))
p <- p + geom_point(shape=24, colour="green", fill="red")
p
```
```{r}
